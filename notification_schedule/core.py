"""Core notification scheduling logic, shared by CLI and Android service."""
...
def run_scheduler(
  ...
  notification_command: str = "claro-notification",
) -> None:
  """Load notification config and schedule all notifications.

  Args:
      config_path: Path to notification_schedule.yaml
      os_impl: Platform OS implementations
      notification_command: Executable for individual notifications (Linux only)
  """
  ...
  for notification_name, notification_config in schedule_config.notifications.items():
    ...
    schedule_notification(
      timer_mgr, notification_name, notification_config, notification_command
    )
