...
stdenv.mkDerivation {
  ...
  # Copy the carlo_app.py script from the source
  ...
  buildPhase = ''
    ...
    cp $src/carlo_app.py app.py
    
    # Substitute the frontend path placeholder
    substituteInPlace app.py \
      --replace-fail '@FRONTEND_PATH@' "$out/share/carlo/frontend"
  ...
  installPhase = ''
    ...
    mkdir -p $out/bin
    mkdir -p $out/share/carlo/frontend
    mkdir -p $out/share/carlo/backend
    
    # Copy frontend assets
    cp -r ${frontend}/* $out/share/carlo/frontend/
    
    # Copy backend source code
    cp -r $src/backend/* $out/share/carlo/backend/
    
    # Install the application entry point script
    cp app.py $out/share/carlo/
    
    # Create wrapper script that uses backend's Python environment
    # The wrapper sets PYTHONPATH to include the backend code
    makeWrapper ${backend}/bin/python $out/bin/carlo \
      --add-flags "$out/share/carlo/app.py" \
      --set PYTHONPATH "$out/share/carlo:${backend}/${python3.sitePackages}:${python3.pkgs.pygobject3}/${python3.sitePackages}:${python3.pkgs.pycairo}/${python3.sitePackages}"
    
    runHook postInstall
  '';
  
  meta = with lib; {
    description = "Carlo AI Assistant - Desktop application";
    longDescription = ''
      Carlo is a personal AI assistant with a chat interface and notifications.
      It combines a FastAPI backend with a React frontend, packaged as a
      desktop application using pywebview.
    '';
    homepage = "https://github.com/zmrocze/carlo";
