...
The interface is designed with classic frontend tools:
 ...
 - tailwind styling and shadcn components are used whenever there's a need, design is modern and pretty
...
We need to figure out how the api token is provided and securely stored for use by the application.
...
The query to llm is performed with memory context from zep. Every new message from user or chat response is sent to zep (single thread).
The most recent conversation history is stored in an ephemeral session memory, to be displayed in the app.
There is a single, sequential conversation thread, for every instance of the app.
...
The notification contains a message from the llm chat bot (how it is constructed is described in config file, fully described below).
Technically this is achieved differently on linux and android.
...
and llm dev prompt, that describes how the model should draft a message shown in the notification.
...
Tests:
testing meaningfull app usage workflows. unit testing the more tricky or complex functions.

Code style:
clear, concise and short, straight to the point.
